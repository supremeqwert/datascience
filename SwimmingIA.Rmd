...
free800 <- mswimmers %>%
  filter(Event == "Swimming Men's 800 metres Freestyle")
```
...
summary(free800$Height)
t.test(free100$Height)$"conf.int"

summary(free800$BMI)
t.test(free100$BMI)$"conf.int"

summary(MOlympics$Height)
summary(MOlympics$BMI)
...
#compares swim medalists to non medalists
ggplot(rioOly, aes(x = BMI, fill = Sex)) +
  geom_histogram(alpha = 0.5) + theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ Medal, scales = "free")

ggplot(rioOly, aes(x = Height, fill = Sex)) +
  geom_histogram(alpha = 0.5) + theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  facet_wrap(~ Medal, scales = "free")
